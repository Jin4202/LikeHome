...
import { searchByLocation } from '../Search/Search';

function NavBarBottom() {
  function search() {
    let str = document.getElementById('location_input').value;
      ...
      <div className='h-full sm:w-3/4 md:w-2/4 mx-auto bg-white'>
        <div className='grid sm:grid-cols-2 md:grid-cols-4 border border-slate-900 h-full divide-x-[3px] sm:divide-y-[3px]'>
          <div className='flex h-full w-full justify-center items-center mr-2 ml-2 hover:shadow-xl'>
            <SearchIcon />
            <input className='h-full w-full outline-none mr-2 ml-2 text-center' placeholder='Search' id='location_input'></input>
          </div>
          ...
          <div className='flex h-full w-full justify-center items-center hover:shadow-xl'>
            <SearchIcon />
            <button onClick={(event) => search()} className='h-full w-full outline-none mr-2 ml-2 text-center'></button>
          </div>
        </div>
      </div>
    </div>
